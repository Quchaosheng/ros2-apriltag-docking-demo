...
bool TaskPolicy::action_active() const noexcept
{
  return action_active_;
}

void TaskPolicy::mark_active() noexcept
{
  action_active_ = true;
}

void TaskPolicy::mark_idle() noexcept
{
  action_active_ = false;
}

void TaskPolicy::update_guard(bool allowed, double stamp)
{
  guard_allowed_ = allowed;
  guard_stamp_ = stamp;
}

std::pair<bool, std::string> TaskPolicy::can_start(double now) const
{
  if (action_active_) {
...
std::optional<std::string> TaskPolicy::cancel_reason(double now) const
{
  if (!action_active_) {
    return std::nullopt;
  }
...
std::optional<std::string> TaskPolicy::guard_reason(double now) const
{
  if (!guard_required_) {
    return std::nullopt;
  }
  if (guard_allowed_ != std::optional<bool>(true) || !guard_stamp_.has_value()) {
    return "GUARD_DENIED";
  }
