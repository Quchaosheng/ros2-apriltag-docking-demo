...
class TestFullCppSimulation(unittest.TestCase):
    """Exercise both C++ bridges in the complete Gazebo graph."""

    @classmethod
    def setUpClass(cls):
        rclpy.init()
        cls.node = Node('full_cpp_launch_test')
        cls.pose_seen = Event()
    ...
    def test_cpp_bridges_publish_expected_graph_state(self):
        ...
        self.fail(
            'full C++ bridge graph did not publish admitted poses and IDLE '
            'within 180 seconds'
        )


@launch_testing.post_shutdown_test()
class TestFullCppShutdown(unittest.TestCase):
    """Verify that both selected C++ bridge processes exit cleanly."""

    def test_processes_exit_cleanly(self, proc_info):
        for process in ('tag_pose_bridge_cpp', 'docking_task_bridge_cpp'):
