...
class TestCppTagBridgeSimulation(unittest.TestCase):
    """Exercise the C++ tag bridge with the Python task bridge."""

    @classmethod
    def setUpClass(cls):
        rclpy.init()
        cls.node = Node('cpp_tag_bridge_launch_test')
        cls.pose_seen = Event()
    ...
    def test_cpp_tag_bridge_publishes_admitted_pose(self):
        ...
        self.fail(
            'C++ tag bridge did not publish an admitted dock pose '
            'within 180 seconds'
        )


@launch_testing.post_shutdown_test()
class TestCppTagBridgeShutdown(unittest.TestCase):
    """Verify that both project-owned bridge processes exit cleanly."""

    def test_processes_exit_cleanly(self, proc_info):
        for process in ('tag_pose_bridge_cpp', 'docking_task_bridge'):
